...
<template>
  <div v-if="conn" class="space-y-4">
    ...
    <div class="flex items-start gap-2.5 p-2.5 rounded-md bg-elevated border border-stroke">
      ...
      <div>
        <div class="text-xs font-medium text-ink">Bypass CORS</div>
        <p class="text-[11px] text-ink-faint mt-0.5 leading-relaxed">Route all requests through the built-in dev proxy. Enable if you see CORS errors.</p>
      </div>
    </div>
